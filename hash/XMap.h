...
//                  void (*deleteKeys)(xMap<K, V>* pMap)) : hashCode(hashCode), loadFactor(loadFactor),
...
template <class K, class V>
inline xMap<K, V>::xMap(int (*hashCode)(K &, int), float loadFactor,
                        bool (*valueEqual)(V &, V &), void (*deleteValues)(xMap<K, V> *),
                        bool (*keyEqual)(K &, K &), void (*deleteKeys)(xMap<K, V> *)) :
                        hashCode(hashCode), loadFactor(loadFactor), valueEqual(valueEqual),
                        deleteValues(deleteValues), keyEqual(keyEqual), deleteKeys(deleteKeys), count(0), capacity(10)
{
  table = new DLinkedList<Entry*>[capacity];
}

template <class K, class V>
xMap<K, V>::xMap(const xMap<K, V> &map) : capacity(0), count(0), table(nullptr), deleteKeys(nullptr), deleteValues(nullptr)
{
  // TODO YOUR CODE IS HERE
  // cout << "copy constructor" << endl;
  // cout << "deleteKeys set to nullptr" << endl;
  // cout << "copyMapFrom" << endl;
  copyMapFrom(map);
...
V xMap<K, V>::put(K key, V value) {
  ...
  bool keyFound = false;
  // TODO YOUR CODE IS HERE
  clash = table[index].empty()? false : true;
...
V xMap<K, V>::remove(K key, void (*deleteKeyInMap)(K)) {
  ...
  for(typename DLinkedList<Entry*>::Iterator it = table[index].begin(); it != table[index].end(); it++){
    ...
    if(keyEQ(entry->key, key)){
      ...
      V value = entry->value;
      table[index].removeItem(entry, &deleteEntry);
      count--;
      return value;
    }
  }
